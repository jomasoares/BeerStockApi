...
import one.digitalinnovation.JoaoBeerStock.builder.BeerDTOBuilder;
...
import static one.digitalinnovation.JoaoBeerStock.utils.JsonConvertionUtils.asJsonString;
...
public class BeerControllerTest {
    ...
    private static final String BEER_API_SUBPATH_TYPE_URL = "/type";
    private static final String BEER_API_SUBPATH_STOCK_BELLOW_URL = "/stock/bellow";
    ...
    void whenPATCHIsCalledToDecrementLowerThanZeroThenBadRequestStatusIsReturned() throws Exception {
        ...
        when(beerService.decrement(VALID_BEER_ID, quantityDTO.getQuantity())).thenThrow(BeerEmptyStockException.class);
    ...
    @Test
    void whenGETListWithBeersByTypeIsCalledThenOkStatusIsReturned() throws Exception {
        // given
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO();

        //when
        when(beerService.listByType(beerDTO.getType())).thenReturn(Collections.singletonList(beerDTO));

        // then
        mockMvc.perform(MockMvcRequestBuilders.get(BEER_API_URL_PATH + BEER_API_SUBPATH_TYPE_URL + "/" + beerDTO.getType())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].name", is(beerDTO.getName())))
                .andExpect(jsonPath("$[0].brand", is(beerDTO.getBrand())))
    ...
    @Test
    void whenGETListWithoutBeersByTypeIsCalledThenOkStatusIsReturned() throws Exception {
        // given
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO();

        //when
        when(beerService.listByType(beerDTO.getType())).thenReturn(Collections.emptyList());
    ...
    @Test
    void whenGETListBeersWithStockBellowIsCalledThenOkStatusIsReturned() throws Exception {
        // given
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO();

        //when
        when(beerService.listStockBellow(5)).thenReturn(Collections.singletonList(beerDTO));

        // then
        mockMvc.perform(MockMvcRequestBuilders.get(BEER_API_URL_PATH + BEER_API_SUBPATH_STOCK_BELLOW_URL + "/5")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].name", is(beerDTO.getName())))
                .andExpect(jsonPath("$[0].brand", is(beerDTO.getBrand())))
    ...
    @Test
    void whenGETListBeersWithoutStockBellowIsCalledThenOkStatusIsReturned() throws Exception {

        //when
        when(beerService.listStockBellow(12)).thenReturn(Collections.emptyList());

        // then
        mockMvc.perform(MockMvcRequestBuilders.get(BEER_API_URL_PATH + BEER_API_SUBPATH_STOCK_BELLOW_URL + "/12")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    }
}
